...
import { Pencil, Loader2, Search, Star, XCircle } from "lucide-react";
...
export default function FeaturedPage() {
  ...
  async function setFeatured(p: Prod, featured: boolean) {
    if (!featured && !confirm(`Remove "${p.name}" from Featured Products?`)) return;
    try {
    ...
    <div>
      <div className="mb-6">
        <h1 className="flex items-center gap-2 text-2xl font-semibold text-foreground">
          <Star size={22} className="text-amber-500" /> Featured Products
        </h1>
        <p className="mt-1 text-sm text-grey-500">{featured.length} product(s) featured on the homepage</p>
      </div>

      <div className="mb-4 rounded-xl border border-grey-200 bg-white px-4 py-3 text-sm text-grey-600">
        Featured products appear in the{" "}
        <strong className="font-semibold text-foreground">Featured Products</strong> section on the
        homepage. Star a product below to feature it, or un-star to remove it.
      </div>
      ...
      {loading ? (
        ...
        <div className="space-y-8">
          {/* Currently featured */}
          <Group
            title={`Featured (${featured.length})`}
            empty="No featured products yet. Star a product below to feature it."
            rows={featured}
            renderActions={(p) => (
          ...
          <Group
            ...
            renderActions={(p) => (
              <div className="flex items-center justify-end">
                <button
                  onClick={() => setFeatured(p, true)}
                  className="flex items-center gap-1.5 rounded-lg border border-grey-200 px-3 py-1.5 text-xs font-semibold text-grey-600 hover:border-amber-400 hover:text-amber-600"
                >
